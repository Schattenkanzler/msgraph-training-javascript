...
async function makeGraphCallAsync() {
  ...
  try {
    // Abrufen der nächsten Ereignisse im Kalender
    const startDateTime = new Date().toISOString();
    const endDateTime = new Date(Date.now() + 7 * 24 * 60 * 60 * 1000).toISOString(); // Nächsten 7 Tage
    const events = await _userClient
      .api('/me/calendarview')
      .query({
    ...
    if (events && events.value) {
      events.value.forEach((event) => {
        console.log(`Ereignis: ${event.subject}`);
        console.log(`Start: ${event.start.dateTime}`);
        console.log(`Ende: ${event.end.dateTime}`);
        console.log('---');
      });
    } else {
      console.log('Keine Ereignisse gefunden.');
    }
  } catch (error) {
